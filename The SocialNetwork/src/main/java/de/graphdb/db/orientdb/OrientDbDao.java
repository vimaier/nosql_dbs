...
public class OrientDbDao implements GraphDBInterface {
  ...
  public UserDTO updateUser(UserDTO user) {
    ...
    if (g != null) {
      try {
        ...
        if (toUpdate == null) {
          log.error("Tried to update a not existing user for id: "
              + user.getId());
          g.commit();
          return null;
        } else {
    ...
    if (g != null) {
      try {
        ...
        if (collectFriends == null) {
          ...
          /*
           * get all realtionships with the laben 'FRIENDS' and add the nodes to
           * the result list
           */
          for (Vertex toAdd : collectFriends.getVertices(Direction.BOTH,
              UserDTO.RELATIONSHIPS.FRIENDS.toString())) {
   ...
   * Valid keyword are:
   * 
   * - Forename - Lastname - Forename Lastname - Lastname Forename
   * 
  ...
  public Collection<UserDTO> findUsers(String suchbegriff) {
    ...
    try {
      ...
      String query = "select from V WHERE forename.toLowerCase() LIKE :forename OR surname.toLowerCase() LIKE :surname"
          + toAppend;
      ...
      log.debug("Found " + toReturn.size() + " users for keyword: "
          + suchbegriff);
  ...
  private boolean createRelationShip(UserDTO user, UserDTO user2,
    ...
    if (user.getId() == null || user.getId().isEmpty()) {
      log.debug("Invalid userId " + user.getId()
          + " -> abort create realtionship");
      return false;
    }

    if (user.getId() == null || user2.getId().isEmpty()) {
      log.debug("Invalid user2Id " + user2.getId()
          + " -> abort create realtionship");
      return false;
    }
    ...
    if (g != null) {
      try {
        ...
        /*
         * create a new edge between the given objects and set the current
         * systemtime as date of creation
         */
        Edge e = g.addEdge(null, userV, userF, relationship.toString());
        e.setProperty("dateOfCreation",
        ...
        log.error("Couldn't create relationship '" + relationship
            + "' between " + user.getId() + " and " + user2.getId(), e);
        g.rollback();
      } finally {
        g.shutdown();
      }
  ...
  private boolean deleteRelationship(UserDTO user, UserDTO user2,
      RELATIONSHIPS relationship) {
    if (user == null || user2 == null) {
    ...
    if (g != null) {
      try {
        ...
        log.error("Couldn't delete relationship '" + relationship
            + "' between " + user.getId() + " and " + user2.getId(), e);
        g.rollback();
      } finally {
        g.shutdown();
  ...
  public Collection<UserDTO> findFriendsOfFriends(UserDTO user) {
    /*
     * This is not really the requested implementation. It should be replaced
     * with a 'traverse' query.
     * 
     * 
     * A proper way would be to replace the parameter 'friends' with a single
     * user object and to use the orientdb traverse api to retrieve all desired
     * users
     */
    ...
    try {
      ...
      Collection<UserDTO> toReturn = new ArrayList<UserDTO>();
      for (ODocument id : new OSQLSynchQuery<ODocument>(
          "select from (traverse out_FRIENDS,in from " + user.getId()
              + " while $depth <= 4) where $depth = 4 AND @class = 'V'")) {
      ...
      log.error(
          "Couldn't traverse friend of friend if the user: " + user.getId(), e);
      g.rollback();
    } finally {
      g.shutdown();
    }
  ...
  public UserDTO loginUser(LoginDTO login) {
    ...
    if (login.getPassword() == null || login.getPassword().isEmpty()) {
      log.debug("LoginObject contains an invalid password: "
          + login.getPassword());
      return null;
    }

    if (login.getMailadress() == null || login.getMailadress().isEmpty()) {
      log.debug("LoginObject contains an invalid mailadress "
          + login.getMailadress());
      return null;
    }
    ...
    try {
      ...
      /*
       * start security control. The result of this query should be a single
       * result
       */
      if (result.size() == 0) {
        log.debug("Couldn't find a user for mail: " + login.getMailadress()
            + " and password: " + login.getPassword());
      }

      if (result.size() > 1) {
        log.debug("Found multiple entries for users with mail: "
            + login.getMailadress() + " and password: " + login.getPassword());
      }
      ...
      log.error("Couldn't login with credentials: " + login.getMailadress()
          + " : " + login.getPassword(), e);
      g.rollback();
    } finally {
      g.shutdown();
